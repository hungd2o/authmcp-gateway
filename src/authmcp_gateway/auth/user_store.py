...
def init_database(db_path: str):
    ...
    with get_db_connection(db_path) as conn:
        ...
        cursor.execute("PRAGMA table_info(user_personal_access_tokens)")
        existing_pat_columns = {row[1] for row in cursor.fetchall()}
        if "access_token" not in existing_pat_columns:
            cursor.execute("ALTER TABLE user_personal_access_tokens ADD COLUMN access_token TEXT")
        if "no_expire" not in existing_pat_columns:
            cursor.execute(
                "ALTER TABLE user_personal_access_tokens ADD COLUMN no_expire BOOLEAN DEFAULT 0"
            )
    ...
    with get_db_connection(db_path) as conn:
        ...
        cursor.execute(
            ...
            INSERT INTO user_personal_access_tokens
                (user_id, token_name, token_jti, expires_at, lifetime_minutes, access_token, no_expire)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                user_id,
    ...
    with get_db_connection(db_path) as conn:
        ...
        cursor.execute(
            ...
            SELECT id, user_id, token_name, token_jti, expires_at, lifetime_minutes,
                   last_used_at, last_used_ip, revoked_at, created_at, updated_at, no_expire
    ...
    with get_db_connection(db_path) as conn:
        ...
        cursor.execute(
            ...
            SELECT pat.id, pat.user_id, pat.token_name, pat.token_jti,
                   pat.expires_at, pat.lifetime_minutes, pat.no_expire,
                   pat.last_used_at, pat.last_used_ip, pat.revoked_at,
                   pat.created_at, pat.updated_at,
                   CASE
                       WHEN pat.access_token IS NOT NULL AND pat.access_token != '' THEN 1
                       ELSE 0
                   END AS can_view_token,
                   u.username
